...
const command: Record<string, string>= {
    ...
    help: '[오레하|아르고스|발싱(발탄:싱글[ㅂㅌㅅㄱ]|발탄노말|발탄하드|비싱(비아키스:싱글[ㅂㅇㅅㄱ])|비아노말|비아하드|쿠싱(쿠크세이튼:싱글[ㅋㅅ])|쿠크|싱브(아브렐슈드:싱글)|노브|하브|노칸|하칸|싱칸(일리아칸:싱글)|노양겔|하양겔|싱양겔(카양겔:싱글)|상노탑|상하탑|상싱탑(상아탑:싱글)|노멘|하멘|에키드나노말|에키드나하드|에기르노말|에기르하드|베히모스]',
...
export const raidName = async (str: string, goldOptions: string) => {
    ...
    if (rewardRaidName !== 0) {
        ...
        // } else {
            for (const reward of rewards) {
                const diffTitle = (rewardLength > 1) ? `[${reward.raid_phase}관문] 입장레벨: ${reward.raid_minItemLevel}${reward.raid_maxItemLevel ? `, 골드획득불가: ${reward.raid_maxItemLevel}\n` : '\n'}` : '';
                rewardArr.push(`${diffTitle}골드: ${reward.raid_rewardGold}G${reward.raid_rewardItem?`\n재료: ${reward.raid_rewardItem}`:''}${reward.raid_bonus_amount?`\n\n[더보기]\n재료: ${reward.raid_bonus_amount}\n`:''}${reward.raid_bonus_amountGold?`(필요: ${reward.raid_bonus_amountGold} 골드)`:''}\n`);
                totalGold += parseInt(reward.raid_rewardGold);
