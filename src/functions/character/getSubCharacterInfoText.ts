    ...
    try {
        ...
        for(const tmp of data) {
            if(characterServer[0].ServerName === tmp.ServerName)
                characterListArr.push({combatLevel: tmp.CharacterLevel, itemLevel: tmp.ItemAvgLevel, textStr: `[${tmp.CharacterClassName}] Lv ${tmp.CharacterLevel.toString().padStart(2, '0')}  ${tmp.CharacterName}  (${tmp.ItemAvgLevel})`})
        }
